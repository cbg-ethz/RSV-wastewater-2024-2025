...
"""
Prepare TSV file in the format for the genetic diversity analysis.
Analyze annotated vcf files
"""

THRESHOLD_VALUE = 0.0 # minimal frequency value
RARE_MUTATION_LIMIT_DAYS = 1 # to be included mutation has to appear for at least RARE_MUTATION_LIMIT_DAYS days
COVERAGE_THRESHOLD = 10  # coverage depth below which mutation is treated as missing value
...
#            print(new_aa)
    ...
    tsv_samples_locations = pd.read_csv(timeline_tsv, sep='\t', header=0,
                                        usecols=["ID", "date", "location", "protocol"])
#    print(tsv_samples_locations)
    tsv_samples_locations = tsv_samples_locations[["ID", "date", "location", "protocol"]]
    tsv_samples_locations.columns = ["submissionId", "date", "location", "reference"]
    ...
    tsv_samples_locations = tsv_samples_locations[(tsv_samples_locations['reference'] == subtype)
                                                  | (tsv_samples_locations['reference'] == "both")]
    tsv_samples_locations["reference"] = subtype


    tsv_samples_locations.to_csv(
        f'timeline_mutation_{reference}_GEN_DIV.tsv', sep='\t',
...
if __name__ == '__main__':
    ...
    parser.add_argument('--timeline_tsv', help='path to SAMPLES.TSV file')
