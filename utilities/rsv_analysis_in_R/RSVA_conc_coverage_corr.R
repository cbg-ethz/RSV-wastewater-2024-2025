...
# Coverage
coverage_a <- read.csv('../../RSV_results/data_analysis/data/RSVA_2024_2025/coverage/collected_rsv_coverage_all_2024_2025_EPI_ISL_412866.tsv')
...
coverage_a <- coverage_a[c("pos", "coverage", "Date", "location")]

length_rsva_genome <- max(coverage_a$pos) #total number of genome bases
F_coordinates <- c(5697, 7421)
F_length = F_coordinates[2] - F_coordinates[1] + 1
coverage_a_plotting <- coverage_a %>%
  group_by(location, Date) %>%
  mutate(covered = sum(coverage >= 10) / length_rsva_genome) %>%
...
coverage_a_plotting <- coverage_a_plotting %>%
  full_join(
    rsva_samples %>%  rename(Date = date),
    by = c("location", "Date")
  ) %>%
  replace_na(list(
    covered = 0,
...
coverage_subtype_conc <- inner_join(coverage_a_plotting, concentration_subtypes, by = c("location", "Date"))[c('Date', 'location','covered','F_covered', 'mean_pos_cov', 'RSV_A')]
...
robust_exponential_models <- coverage_subtype_conc %>%
  group_by(location) %>%
  summarise(
    model = list(lmrob(ln_covered_modified ~ 0 + RSV_A)),
    slope = coef(model[[1]])[1],
    spearman_rho = cor.test(RSV_A, covered, method = 'spearman')$estimate,
    p_value = cor.test(RSV_A, covered, method = 'spearman')$p.value,
    max_RSVA = max(RSV_A, na.rm = TRUE)
  )
...
exp_curve_data <- robust_exponential_models %>%
  group_by(location) %>%
  tidyr::expand(cc_dat = seq(0, max_RSVA, max_RSVA/20)) %>%  # Expand grid for smooth curve
...
annotation_df <- robust_exponential_models %>%
  ...
  mutate(
    x = max(coverage_subtype_conc$RSV_A),  # or dynamically max(coverage_subtype_conc$`RSV-A`)
    y1 = 0.1,
    y2 = 0.02,
...
plot_covered <- ggplot(coverage_subtype_conc, aes(x = RSV_A, y = covered, color = location)) +
  geom_point() +
  ylim(c(0,1)) +
  
  geom_point(aes(x = RSV_A, y = covered, color = location, shape = "Whole genome"), size = 2.5) +
  geom_point(aes(x = RSV_A, y = F_covered, shape = "F gene"), size = 2.5, color='black', alpha = 0.3) +
  geom_line(data = exp_curve_data, aes(x = cc_dat, y = predicted_y, color = location), linetype = "dashed") +

  
  labs(
    title = "", 
    x = "RSV-A, gc/mL wastewater", #"gc/mL wastewater", 
    y = bquote("Covered " >= 10 ~ "×")
  ) +
  theme_minimal()  + 
  theme(
...
correlation_all_locations <-  cor.test(coverage_subtype_conc$RSV_A, 
                                       coverage_subtype_conc$covered,
                                       method = 'spearman')
print(correlation_all_locations)
# coverage_a_plotting contains all RSV-A samples, whereas coverage_subtype_conc contains only the samples for which we have RSV-AB assay measurements
...
# Compare mean viral loads across different locations
